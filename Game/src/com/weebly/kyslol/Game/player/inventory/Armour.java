...
public class Armour extends Inventory{
	...
	public Armour(String name, int Ammount, String description, int dmg){
		this.dmg = dmg;
		try {
			img = ImageIO.read(InventoryItems.class.getResource(URL + "Armour/" + name.toLowerCase() + ".png"));
		} catch (IOException e) {
...
//		Inventory.Items.size()/10);
		int i = 0;
		for (int len = 0; len < Inventory.Armour.size(); len ++){
			if(Inventory.Armour.get(len).getName().equals(name)&&Inventory.Armour.get(len).getDmg()==dmg){
				i = len + 1;
//				continue;
			}
		}
